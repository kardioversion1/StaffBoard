...
const demoState: BoardState = {
  ...
  ui: { density: 'comfortable', view: 'board', draggingNurseId: null, dragTargetZoneId: null, contextMenu: null },
  version: 1,
};
...
interface Store extends BoardState {
  ...
  setUi: (ui: Partial<BoardState['ui']>) => void;
...
export const useStore = create<Store>((set, get) => ({
  ...
  markOffAction: (id) => set((state) => markOff(state, id)),
  setRfAction: (id, rf) => set((state) => setRf(state, id, rf)),
  setStudentTagAction: (id, tag) => set((state) => setStudentTag(state, id, tag)),
  setShiftEndAction: (id, iso) => set((state) => setShiftEnd(state, id, iso)),
  setBreakAction: (id, on, cover) => set((state) => setBreak(state, id, on, cover)),
}));
