...
async function wireControls(){
  ...
  $('#themeToggle').addEventListener('click', ()=>{
    ...
    DB.set(KS.CONFIG, { theme: STATE.theme, accent: STATE.accent }).catch(e=>debug('Config save error: '+e.message));
  });
...
// -------- Init
async function init(){
  const cfg = await DB.get(KS.CONFIG).catch(e=>{debug('Config load error: '+e.message);return null;});
